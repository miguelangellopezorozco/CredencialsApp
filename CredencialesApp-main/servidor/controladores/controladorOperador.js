...
exports.eliminarOperador = async (req, res) => {
  try {
    ...
    if (req.session.operadorId == id) {
      return res
        .status(400)
        .json({ mensaje: "No puedes eliminar tu propio usuario" });
    }
    ...
    await SesionOperador.destroy({
      where: { operador_id: id },
    });

    // luego eliminar operador
    await operador.destroy();

    res.json({ mensaje: "Operador eliminado correctamente" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ mensaje: "Error al eliminar operador" });
  }
};
