...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Modal de PDF
  const pdfPreviewModal = new bootstrap.Modal(
  ...
  function crearElementoDesdeLayout(elemData, editor, dragDrop) {
    ...
    switch (elemData.type) {
      ...
      case "placeholder":
        ...
        if (currentUser && elemData.field) {
          ...
          const valor = currentUser[campo];

          // 🔥 SI ES FOTO → crear imagen
          if (campo === "foto" && valor) {
            element.innerHTML = "";
            ...
            const img = document.createElement("img");
            img.src = valor; // importante el slash inicial
            img.style.width = "100%";
            img.style.height = "100%";
            ...
            element.textContent = valor || elemData.field;
          }
        } else {
    ...
    if (elemData.zIndex !== undefined) {
      element.style.zIndex = elemData.zIndex;
    }
  ...
  function getElementData(element) {
    ...
    data.zIndex = element.style.zIndex || 1;
  ...
  btnPDF.addEventListener("click", async () => {
    ...
    try {
      ...
      // Generar PDF
      const pdfData = await api.guardarPDFCredencial(
        currentUser.id,
  ...
  placeholderButtons.forEach((b) => {
    b.addEventListener("click", () => {
      crearPlaceholder(b.dataset.field);
