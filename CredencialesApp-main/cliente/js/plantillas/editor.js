...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Modal de texto
  const textEditModal = new bootstrap.Modal(
  ...
  const placeholderModal = new bootstrap.Modal(
    document.getElementById("placeholderModal"),
  );
  ...
  const placeholderProperties = document.getElementById(
  ...
  const propPlaceholderFamily = document.getElementById(
    "propPlaceholderFamily",
  );
  ...
  const btnToggleLineOrientation = document.getElementById(
    "btnToggleLineOrientation",
  );
  ...
  if (id) {
    ...
    editorTitle.innerHTML =
      '<i class="bi bi-pencil-square me-2"></i>Editar Plantilla';
    cargarPlantilla(id);
  }
  ...
  function enhanceDragAndDrop() {
    ...
    DragAndDrop.prototype.handleMouseMove = function (e) {
      ...
      if (
        this.isDragging &&
        this.selected &&
        !this.selected.classList.contains("dragging-preview")
      ) {
  ...
  function mejorarSeleccionLinea() {
    ...
    DragAndDrop.prototype.clearSelection = function () {
      ...
      if (
        this.selected &&
        this.selected.getAttribute("data-type") === "linea"
      ) {
    ...
    try {
      ...
      if (plantilla.layout_data) {
        ...
        try {
          ...
          layoutData =
            typeof plantilla.layout_data === "string"
        ...
        if (layoutData.frente && Array.isArray(layoutData.frente)) {
          layoutData.frente.forEach((elem) =>
            crearElementoDesdeLayout(elem, frontEditor, frontDragDrop),
          );
        }

        // Renderizar reverso
        if (layoutData.reverso && Array.isArray(layoutData.reverso)) {
          layoutData.reverso.forEach((elem) =>
            crearElementoDesdeLayout(elem, backEditor, backDragDrop),
          );
        }
      }
    } catch (error) {
  ...
  function crearElementoDesdeLayout(elemData, editor, dragDrop) {
    ...
    switch (elemData.type) {
      ...
      case "placeholder":
        element.setAttribute("data-field", elemData.field);

        if (elemData.align) element.style.textAlign = elemData.align;

        if (elemData.isImage) {
          element.classList.add("element-image-placeholder");
          ...
          element.classList.add("element-text");
          element.textContent = elemData.field;
      ...
      case "rectangulo":
        ...
        if (elemData.fillColor)
          element.style.backgroundColor = elemData.fillColor;
        if (elemData.borderColor)
          element.style.borderColor = elemData.borderColor;
        if (elemData.borderWidth)
          element.style.borderWidth = elemData.borderWidth;
        break;

      case "linea":
        if (elemData.orientation === "vertical") {
          element.classList.add("element-line", "vertical");
          if (elemData.lineColor)
            element.style.borderLeft = `${elemData.lineWidth} ${elemData.lineStyle} ${elemData.lineColor}`;
        } else {
          element.classList.add("element-line", "horizontal");
          if (elemData.lineColor)
            element.style.borderTop = `${elemData.lineWidth} ${elemData.lineStyle} ${elemData.lineColor}`;
        }
        break;
    }
  ...
  function crearPlaceholder(field) {
    ...
    const element = document.createElement("div");
    element.className = "draggable element-placeholder";
    ...
    if (field === "{{foto}}") {
      ...
      const img = document.createElement("img");
      img.src = "../img/placeholder-user.png"; // imagen temporal
      img.style.width = "100%";
      img.style.height = "100%";
      img.style.objectFit = "cover";
    ...
    input.onchange = (e) => {
      ...
      if (file) {
        ...
        reader.onload = (event) => {
          ...
          const editor = activeEditor === "front" ? frontEditor : backEditor;
          const dragDrop =
            activeEditor === "front" ? frontDragDrop : backDragDrop;
  ...
  function handleElementResize(element) {
    ...
    if (
      element.getAttribute("data-type") === "texto" ||
      element.getAttribute("data-type") === "placeholder"
    ) {
  ...
  function actualizarPanelPropiedades(element) {
    ...
    const editorRect = (
      activeEditor === "front" ? frontEditor : backEditor
    ...
    switch (tipo) {
      ...
      case "placeholder":
        ...
        const alignp = element.style.textAlign || "left";
        document.getElementById(`align${capitalize(alignp)}`).checked = true;
      ...
      case "linea":
        ...
        document.getElementById("lineDashed").checked =
          estiloLinea === "dashed";
        document.getElementById("lineDotted").checked =
          estiloLinea === "dotted";
        break;
    }
  }
  ...
  function aplicarPropiedades() {
    ...
    switch (tipo) {
      ...
      case "placeholder":
        ...
        const alignRadiosPlaceholder = document.getElementsByName("textAlign");
        for (const radio of alignRadiosPlaceholder) {
          if (radio.checked) {
            element.style.textAlign = radio.value;
            break;
          }
  ...
  function obtenerDatosEditor() {
    ...
    const elementosFronte = Array.from(
      frontEditor.querySelectorAll(".draggable"),
    );
    const datosFronte = elementosFronte.map(getElementData);

    // Elementos del reverso
    const elementosReverso = Array.from(
      backEditor.querySelectorAll(".draggable"),
    );
  ...
  function getElementData(element) {
    ...
    switch (tipo) {
      ...
      case "placeholder":
        ...
        data.fontFamily = element.style.fontFamily;
        data.color = element.style.color;
        data.isImage = element.querySelector("img") ? true : false;
        data.align = element.style.textAlign;
        break;
